...
SAMA="$COMP_DIR/GCA_002910315.2_ASM291031v2_genomic.fna"

SASA_IC="$COMP_DIR/GCA_000233375.4_ICSASG_v2_genomic.fna"
...
#samtools faidx $COCL
#samtools faidx $SAMA
samtools faidx $SASA_IC

# Run assembly-stats
assembly-stats -t $FINAL_NCBI $SANA $OMYK $SATR $SASA $COCL $SAMA $SASA_IC
...
#stats.sh $SAMA > "${SAMA%.*}".stats.txt
stats.sh $SASA_IC > "${SASA_IC%.*}".stats.txt
