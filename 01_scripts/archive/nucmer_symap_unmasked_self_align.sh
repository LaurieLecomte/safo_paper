...
# srun -p medium -c 12 --mem=100G --time=7-00:00:00 -J nucmer_symap_unmasked_self_align -o log/nucmer_symap_unmasked_self_align_%j.log /bin/sh 01_scripts/nucmer_symap_unmasked_self_align.sh SaFo &
...
nucmer -t $CPU $REF $QUERY --maxmatch --genome -p $OUTPUT_DIR/unmasked_"$SPECIES"_on_SaFo.maxmatch

show-coords -dlTH $OUTPUT_DIR/unmasked_"$SPECIES"_on_SaFo.maxmatch.delta > $OUTPUT_DIR/unmasked_"$SPECIES"_on_SaFo.maxmatch.mum
