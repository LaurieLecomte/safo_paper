...
# parallel -a 02_infos/SaFo_corrected_chrs.txt -j 10 srun -p medium -c 5 --mem=100G --time=7-00:00:00 -J {}_nucmer_self_align_chrom_vs_chrom -o log/nucmer_self_align_chrom_vs_chrom_{}_%j.log /bin/sh 01_scripts/nucmer_self_align_chrom_vs_chrom.sh {} &
...
if [[ ! -d $OUTPUT_DIR/chr_on_chr ]]
then
  mkdir $OUTPUT_DIR/chr_on_chr
fi
...
do
  ...
  nucmer -t $CPU $OUTPUT_DIR/chr/SaFo_"$R_CHR"_masked.fasta $OUTPUT_DIR/chr/SaFo_"$Q_CHR"_masked.fasta --maxmatch -p $OUTPUT_DIR/chr_on_chr/q"$Q_CHR"_on_r"$R_CHR".maxmatch;
  show-coords -dlTH $OUTPUT_DIR/chr_on_chr/q"$Q_CHR"_on_r"$R_CHR".maxmatch.delta > $OUTPUT_DIR/chr_on_chr/q"$Q_CHR"_on_r"$R_CHR".maxmatch.mum;
done


# Combine outputs together
cat $OUTPUT_DIR/chr_on_chr/*_on_r"$R_CHR".maxmatch.mum > $OUTPUT_DIR/chr_on_chr/all_on_r"$R_CHR".maxmatch.mum
