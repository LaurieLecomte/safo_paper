...
setwd("~/lalec31/projets_labo/safo_paper")
...
# Mapped blocks
files <- list.files(path = "synteny/SaFo_SaFo/", 
...
symap_blocks <- read.delim("synteny/SaFo_SaFo/SaFo_self_masked_mindots30_topn2_blocks_interchr_blocks", header = FALSE,
...
for(i in 1:nrow(mapped_blocks)){
  ...
  anchor <- read.delim(paste0("synteny/SaFo_SaFo/", files[i])
                      )
...
# Export as bed file
write.table(x = hom_blocks[,1:5], paste0('synteny/SaFo_SaFo/homolog_blocks_identity_', PCT_IDENTITY, 'win', win.size, '.bed'), row.names = FALSE, col.names = FALSE, 
            sep = "\t", quote = FALSE)
