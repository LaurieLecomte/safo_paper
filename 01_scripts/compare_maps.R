...
# Global variables
output_lg_correspondance = "linkage_maps/linkage_group_correspondance.csv"
minimum_number_of_points = 5 # Minimum number hits between 2 LGs to display in dark
figure_folder = "linkage_maps"
sp1_col=1
sp2_col=8

# Load data.pairs
data.pairs = read.table("linkage_maps/wanted_loci.info")
data.loci = read.table("linkage_maps/markers.fasta.info")
...
data.pairs$V1 <- as.factor(data.pairs$V1)
data.pairs$V8 <- as.factor(data.pairs$V8)
    ...
    for (sp2 in levels(data.pairs[,sp2_col])) {
        if (sp1 != sp2) {
            ...
            plot(data.sp[,(sp1_col+3)], data.sp[,(sp2_col+3)],
                 ...
                 xlab= ifelse(sp1 == 'Claire26final',
                               yes = paste('Current study map - 16,203 markers', "(distances in CM)"),
                               no = paste(sp1, "map (distances in CM)")),
            ...
            for (lg1 in sp1.lgs){
                for (lg2 in sp2.lgs){
                    ...
                    # Color quadrants lighter or darker grey depending on wether
                    ...
                    if (nrow(dd) >= minimum_number_of_points) {
                        ...
                        rect(min.x, min.y, max.x, max.y, col="grey95", border=F)
                    }
